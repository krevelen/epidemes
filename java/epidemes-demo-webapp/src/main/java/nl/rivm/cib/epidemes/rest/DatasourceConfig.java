...
/**
 * {@link DatasourceConfig} JNDI settings inspired by
...
public interface DatasourceConfig extends HibernateJPAConfig
{
	...
	static void defaultJndiDataSource() throws NamingException
	{
		...
		try
		{
			...
			int lastIndex = DatasourceConfig.DATASOURCE_JNDI
					.lastIndexOf( NAME_SEPARATOR );
			if( lastIndex >= 0 )
			{
				Context sub = root;
				for( String name : DatasourceConfig.DATASOURCE_JNDI
			...
			ds.setUser( "SA" );
			ds.setPassword( "" );
			root.bind( DatasourceConfig.DATASOURCE_JNDI, ds );
		} finally
		{
			root.close();
		}
