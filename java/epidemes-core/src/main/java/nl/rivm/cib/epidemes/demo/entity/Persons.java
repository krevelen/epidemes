...
public interface Persons {
	enum HouseholdPosition {
		...
		public Persons.HouseholdPosition shift(final Persons.HouseholdPosition missing) {
			return missing == REFERENT ? (this == PARTNER ? REFERENT : this)
					: (!isAdult() && ordinal() > missing.ordinal() ? values()[ordinal() - 1] : this);
		}
	}
	...
	class CultureRef extends AtomicReference<Comparable> implements Property<Comparable> {
	}

	/** reference of person's household */
	class HouseholdRef extends AtomicReference<Object> implements Property<Object> {
	}
	...
	class Birth extends AtomicReference<BigDecimal> implements Property<BigDecimal> {
	}
	...
	@SuppressWarnings("rawtypes")
	class HomeRegionRef extends AtomicReference<Comparable> implements Property<Comparable> {
	}

	/** reference of person's current home site */
	@SuppressWarnings("rawtypes")
	class HomeSiteRef extends AtomicReference<Comparable> implements Property<Comparable> {
	}

	/** person's current epidemiological compartment for some pathogen */
	class PathogenCompartment extends AtomicReference<MSEIRS.Compartment> implements Property<MSEIRS.Compartment> {
	}

	/** person's current remaining 'resistance' against some pathogen */
	class PathogenResistance extends AtomicReference<Double> implements Property<Double> {
	}
	 ...
	 */
	class VaxCompliance extends AtomicReference<Integer> implements Property<Integer> {
	}

	@SuppressWarnings("rawtypes")
	List<Class<? extends Property>> PROPERTIES = Arrays.asList(
			// list frequently accessed fields first
			Persons.PathogenCompartment.class, Persons.HouseholdRef.class, Persons.HouseholdRank.class,
			Persons.Birth.class, Persons.HomeRegionRef.class, Persons.HomeSiteRef.class,
			Persons.PathogenResistance.class, Persons.Male.class, Persons.CultureRef.class, Persons.VaxCompliance.class,
			Persons.PersonSeq.class);
